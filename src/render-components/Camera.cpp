...
void Camera::zoom(float zoom, int32_t x, int32_t y, sf::RenderWindow* window) {
...
void Camera::moveLeft(int32_t offset) {
    float limitedOffset = getLimitedOffset(
...
float Camera::getMoveStep() const {
    return moveStep_;
}

float Camera::getLimitedOffset(float currentPosition, int32_t offset, int32_t size) const {
    float moveLimit = currentPosition - offset - size * moveOutLimitPortion_;
    float limitedOffset = offset;
    if (moveLimit < 0) {
